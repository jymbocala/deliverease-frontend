...
const TabContent = ({ details, tabCategory, open }) => (
  <div className={`tabcontent ${open === tabCategory ? "block" : "hidden"} p-4 mt-4 border border-gray-300 bg-white shadow-md rounded-lg`}>
    {details}
  </div>
);
...
const SingleLocation = ({ locations, setLocations }) => {
  ...
  const photosTabElements = location && location.imageURL ? (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-2">
      ...
      <dialog id={"my_modal_" + location.id} className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <img src={location.imageURL} className="w-full h-full rounded-lg" alt={location.name} />
          <button className="btn btn-outline btn-accent" onClick={handleDeletePhoto}>
            Delete Photo
          </button>
        </div>
        <form method="dialog" className="modal-backdrop" onClick={(e) => {
          e.stopPropagation(); // Stop the click event from bubbling up to the parent
        }}>
          <button>close</button>
        </form>
  ...
  return (
    <section className="py-20 dark:bg-dark lg:py-[120px] flex justify-center items-center">
      <div className="container mx-auto text-center">
        ...
        <button className="btn btn-outline btn-warning  text-danger py-2 px-4 transform transition duration-500 ease-in-out hover:bg-danger hover:text-white" onClick={(e) => {
          e.stopPropagation(); // Stop the click event from bubbling up to the parent
        ...
        <dialog id={"my_modal_" + location?._id} className="modal modal-bottom sm:modal-middle">
          <div className="modal-box">
            <form method="dialog">
              <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" onClick={(e) => {
                e.stopPropagation(); // Stop the click event from bubbling up to the parent
            ...
            <button className="transform transition duration-500 ease-in-out btn btn-outline btn-error" onClick={(e) => {
              e.stopPropagation(); // Stop the click event from bubbling up to the parent
        ...
        <div className="flex flex-wrap justify-center">
          ...
          <div className="w-full lg:w-[30%] px-4 mx-auto">
            ...
            <div className="flex flex-row justify-center">
              <a onClick={() => setOpen("details")} className={`cursor-pointer px-2 py-1 text-sm font-medium md:text-base rounded-full ${open === "details" ? "bg-primary text-white" : "text-body-color hover:bg-primary hover:text-white dark:text-dark-6 dark:hover:text-white"}`}>Details</a>
              <a onClick={() => setOpen("photos")} className={`cursor-pointer px-2 py-1 text-sm font-medium md:text-base rounded-full ${open === "photos" ? "bg-primary text-white" : "text-body-color hover:bg-primary hover:text-white dark:text-dark-6 dark:hover:text-white"}`}>Photos</a>
            </div>
            <TabContent details={details} tabCategory="details" open={open} />
            <TabContent details={photosTabElements} tabCategory="photos" open={open} />
            <div className="mt-4">
              <Link to="/locations" className="bg-secondary text-white py-2 px-4 rounded transform transition duration-500 ease-in-out hover:bg-primary hover:scale-105">
                Locations
              </Link>
            </div>
          </div>
